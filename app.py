...
def simulate_training():
    return train(use_aoa, num_layers, dropout, learning_rate, batch_size, num_cores, epochs_aoa, population_size, epochs_lstm)
...
with col1:
    ...
    st.subheader("Paramètres AOA")
    use_aoa = st.checkbox("Optimisation AOA", value=True)
    if use_aoa:
        epochs_aoa = st.slider("Epochs", 1, 100, 5, 1,)
        population_size = st.slider("Population size", 1, 100, 10, 1,)
    else:
        epochs_aoa = 5
        population_size = 10
    st.subheader("Paramètres LSTM")
    batch_size = st.slider("Batch size", 2, 64, 16, 2,
                           help="Nombre de partitions du dataset pour travailler en parallel\r"
                              "attention une valeur trop élevée peut entraîner une  surchauffe du processeur")
    num_cores = st.slider("Nombre de CPU cores", 1, 8, 4, 1,
                          help="Nombre de coeurs processeur à utiliser pour l'entraînementr\r"
                               "attention une valeur trop élevée peut entraîner une  surchauffe du processeur")
    epochs_lstm = st.slider("Epochs", 1, 100, 30, 1,)

    if not use_aoa:
        num_layers = st.slider("Number of layers", 1, 3, 2)
        dropout = st.slider("Dropout", 0.0, 0.3, 0.0, 0.05)
        learning_rate = st.slider("Learning rate", 0.0001, 0.005, 0.0015, 0.00001, format="%.5f")
    else:
        num_layers = 2
        dropout = 0.0
